...
export const EnhancedHeroSection: React.FC = () => {
  ...
  const stats = [
    { label: "Active Creators", value: "12,847+", growth: "+15.2%" },
    { label: "NFTs Minted", value: "421,071", growth: "+8.7%" },
    { label: "Creator Earnings", value: "$2.1M+", growth: "+24.3%" },
    { label: "Community Members", value: "2,847", growth: "+12.1%" }
  ];
      ...
      <div className="relative container mx-auto px-4 pt-32 pb-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-6xl mx-auto animate-fade-in animation-delay-500">
          {stats.map((stat, index) => (
            <ModernCard 
              key={index}
              variant="filled"
              className="text-center p-6 hover:scale-105 transition-transform duration-300 bg-background/20 backdrop-blur-sm"
            >
              <div className="text-3xl font-bold text-white mb-2">{stat.value}</div>
              <div className="text-sm text-gray-400 mb-2">{stat.label}</div>
              <div className="inline-flex items-center px-2 py-1 rounded-full bg-web3-green/20 text-web3-green text-xs font-medium">
                <div className="w-1.5 h-1.5 bg-web3-green rounded-full mr-2" />
                {stat.growth}
              </div>
            </ModernCard>
          ))}
        ...
        <div className="flex justify-center mt-16 animate-fade-in animation-delay-600">
          <div className="relative">
            <div className="w-64 h-64 bg-gradient-to-br from-web3-cyan/20 to-web3-purple/20 rounded-full flex items-center justify-center backdrop-blur-sm border border-web3-cyan/30">
              <Award size={120} className="text-web3-cyan opacity-80" />
            </div>
            <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 bg-background/80 backdrop-blur-sm border border-border rounded-lg px-4 py-2">
              <span className="text-sm text-foreground font-medium">Tech Meerkat with VR & AR</span>
            </div>
          </div>
        </div>
      </div>
